...
;;
(defmethod log :message [{:keys [from to message]}]
...
;;
(defmethod log :file [{:keys [from to] {filename :name} :file}]
...
;;
(defmethod log :end [{:keys [from to]}]
...
(defmethod enqueue ::full [_ _]
  (throw (java.lang.RuntimeException. "the queue is full")))
...
(defmethod dequeue ::empty [_]
  (throw (java.lang.RuntimeException. "the queue is empty")))
...
(defmulti route (juxt :uri :method))
